...
- **Synchronous dispatch.** A round waits for its subagents (implement, code-review, fix) before arming and ending. Rounds are strictly serialized, and a fresh round never sees a running subagent: every CR it encounters is fully pushed, so babysit judges CI/forge state alone.
